<template>
  <IonGrid>
    <p class="TitleP">List of all members who have created a Sponsorship Partnership advert</p>
    ...
    <IonRow class="NONscrollingRow">
      ...
      <IonRow class="scrollingRow" ref="scrollableContainer">
        ...
        <IonRow class="TitleRow">
          ...
          <IonCol class="sponsorshipPartnershipTitleCol topBorder" @click="sortMembers('sponsorshipPartnershipTitle')">Sponsorship Partnership Title <IonIcon :icon="sortIcon('sponsorshipPartnershipTitle')" class="sort-icon" /></IonCol>
          <IonCol class="sponsorshipPartnershipSeenCol topBorder" @click="sortMembers('sponsorshipPartnershipSeen')">Sponsorship Partnership Seen <IonIcon :icon="sortIcon('sponsorshipPartnershipSeen')" class="sort-icon" /></IonCol>
          <IonCol class="sponsorshipPartnershipListViewCol topBorder" @click="sortMembers('sponsorshipPartnershipListView')">Sponsorship Partnership List View <IonIcon :icon="sortIcon('sponsorshipPartnershipListView')" class="sort-icon" /></IonCol>
          <IonCol class="CostCol topBorder" @click="sortMembers('cost')">Sponsorship Partnership Cost <IonIcon :icon="sortIcon('cost')" class="sort-icon" /></IonCol>
          <!-- Sponsorship Partnership Section -->
        ...
        <IonRow v-for="member in paginatedMembers" :key="member.id" class="DataRow" :class="{ selected: selectedRow === member.id }" @click="selectRow(member.id)">
          ...
          <!-- Sponsorship Partnership Section -->
          <IonCol class="sponsorshipPartnershipTitleCol">{{ member.sponsorshipPartnershipTitle }}</IonCol>
          <IonCol class="sponsorshipPartnershipSeenCol">{{ member.sponsorshipPartnershipSeen }}</IonCol>
          <IonCol class="sponsorshipPartnershipListViewCol">{{ member.sponsorshipPartnershipListView }}</IonCol>
...
// back button does not wor, fix itk
    ...
    //  * Computes the total price of all members.
      ...
      const totalsponsorshipPartnershipSeen = computed(() => filteredMembers.value.reduce((sum, member) => sum + member.sponsorshipPartnershipSeen, 0));
      const totalsponsorshipPartnershipListView = computed(() => filteredMembers.value.reduce((sum, member) => sum + member.sponsorshipPartnershipListView, 0));
      ...
      const exportTable = () => {
        const csvContent = [
          [
            ...
            'Sponsorship Partnership Title', 'Sponsorship Partnership Seen', 'Sponsorship Partnership List View', 'Cost',
            //  Sponsorship Partnership
          ...
          ...filteredMembers.value.map(member => [
            ...
            //  Sponsorship Partnership
            member.sponsorshipPartnershipTitle, member.sponsorshipPartnershipSeen, member.sponsorshipPartnershipListView, member.cost, 
            //  Sponsorship Partnership
            ...
            member.connections, member.noEmployees, member.expiringDate, member.timeLeftTillExpiry, member.bookingDate, member.joined, member.lastLoggedIn, member.noLoggedIn, member.fullProfileSeen
          ])
        ]
          .map(e => e.join(","))
          .join("\n");
                ...
                <tbody>
                  ${paginatedMembers.value.map((member, index) => `
                    <tr class="${index % 2 === 0 ? 'even-row' : 'odd-row'}">
                      ...
                      // Pitching section
                      <td>${member.sponsorshipPartnershipTitle}</td>
                      <td>${member.sponsorshipPartnershipSeen}</td>
                      <td>${member.sponsorshipPartnershipListView}</td>
                      <td>${member.cost}</td>
                      // Pitching section
                  ...
                  <tr class="TotalRow">
                    ...
                    // Pitching section
                    <td></td>
                    <td>${totalsponsorshipPartnershipSeen}</td>
                    <td>${totalsponsorshipPartnershipListView.value}</td>
                    <td></td>
                    <td></td>
                    <td></td>
